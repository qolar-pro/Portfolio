 ...
 *   data-cursor="view"  — becomes a labeled disc over project media
 *   data-cursor="text"  — thin beam for copy-selection zones
 * Renders nothing on coarse pointers. Also feeds journey.pointer.
 */
export default function Cursor() {
  ...
  useEffect(() => {
    ...
    const applyState = (next: CursorState) => {
      ...
      state = next;
      // difference blending inverts the cursor over the display type;
      // the labeled VIEW disc needs normal compositing to stay readable
      ...
      const cfg = {
        ...
        view: { size: 88, borderColor: 'rgba(255,79,216,0.9)', bg: 'rgba(3,3,9,0.55)', labelOp: 1, dotScale: 0 },
    ...
    // squash & stretch: the ring elongates along its direction of travel
    let lastX = 0;
    let lastY = 0;
    ...
    // press feedback lives on the dot so it never fights the stretch transform
  ...
  return (
    <div ref={wrapRef} aria-hidden className="pointer-events-none fixed inset-0 z-[200]" style={{ mixBlendMode: 'difference' }}>
      <div
        ref={dotRef}
        className="absolute -top-[3px] -left-[3px] h-1.5 w-1.5 rounded-full bg-fog opacity-0"
      />
      <div
        ref={ringRef}
        ...
        style={{
          ...
          // CSS `translate` centers the ring; GSAP owns `transform` for x/y/scale,
          // and the two compose — so the ring stays centered at every size.
          translate: '-50% -50%',
        }}
      >
        <span
          ref={labelRef}
          className="font-mono text-[9px] font-medium uppercase tracking-[0.2em] text-fog opacity-0"
        >
          View
        </span>
      </div>
